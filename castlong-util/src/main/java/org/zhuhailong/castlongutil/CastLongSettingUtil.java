...
/**
 * @author hailong
 * @date 2020年08月22日 9:01 PM
 * <p>
 * You never know what you can do until you try !
 * ----------------------------------------------------------------
 */
public class CastLongSettingUtil {
    ...
    public static void start(AppCompatActivity activity, int requestCode) {
        ...
        if (MARK.contains("huawei")) {
            intent = huaweiApi(activity);
        } else if (MARK.contains("xiaomi")) {
            intent = xiaomiApi(activity);
        } else if (MARK.contains("oppo")) {
            intent = oppoApi(activity);
        } else if (MARK.contains("vivo")) {
            intent = vivoApi(activity);
        } else if (MARK.contains("meizu")) {
            intent = meizuApi(activity);
        } else {
            intent = defaultApi(activity);
        }
        try {
